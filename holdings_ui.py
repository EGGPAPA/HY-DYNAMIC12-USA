...
def _headers():
    ...
    if _github_pat():
        h["Authorization"] = f"Bearer {_github_pat()}"
    return h
...
def _load_holdings():
    ...
    data = r.json()
    rows = json.loads(base64.b64decode(data["content"]).decode("utf-8") or "[]")
...
def _holding_table(rows, label):
    ...
    for r in rows:
        ...
        view.append({"구분": "ETF" if _asset_type(r) == "ETF" else "개별종목", "티커": r.get("ticker", ""),
                     "종목명": r.get("name", ""), "평균매수가($)": round(avg, 4), "수량": qty,
                     "투입금액($)": round(avg * qty, 2), "손절(-3%)": round(avg * .97, 2),
...
def render_holdings_tab():
    ...
    try:
        ...
        st.error(str(e)); rows, sha = [], None
    active = [x for x in rows if str(x.get("status", "holding")).lower() != "closed" and x.get("enabled", True)]
    stocks = [x for x in active if _asset_type(x) == "STOCK"]
    etfs = [x for x in active if _asset_type(x) == "ETF"]
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("전체 보유", len(active)); c2.metric("개별종목", len(stocks)); c3.metric("ETF", len(etfs)); c4.metric("GitHub 저장", "준비됨" if _github_pat() else "PAT 미설정")
    ...
    with st.form("hy_holding_buy_form", clear_on_submit=False):
        ...
        name = b.text_input("종목명", key="hy_hold_name").strip()
        c, d = st.columns(2)
        price = c.number_input("실제 체결 매수가($)", min_value=0.0, step=.01, format="%.4f", key="hy_hold_price")
        qty = d.number_input("매수 수량", min_value=0.0, step=1.0, format="%.4f", key="hy_hold_qty")
    ...
    if submitted:
        if not ticker or price <= 0 or qty <= 0:
            ...
            try:
                rows, sha = _load_holdings(); idx, old = _find(rows, ticker); now = datetime.now(timezone.utc).isoformat()
                kind = "ETF" if asset_type == "ETF" else "STOCK"
                if old is None:
                    new_avg, new_qty = float(price), float(qty)
                    rows.append({"ticker": ticker, "name": name or ticker, "asset_type": kind, "mode": "holding", "status": "holding",
                                 "average_price": round(new_avg, 6), "quantity": round(new_qty, 6), "stop_loss_pct": 3, "enabled": True, "updated_at": now})
                else:
                    old_avg, old_qty = float(old.get("average_price", 0) or 0), float(old.get("quantity", 0) or 0)
                    new_qty = old_qty + float(qty); new_avg = ((old_avg * old_qty) + float(price) * float(qty)) / new_qty
                    old.update({"name": name or old.get("name") or ticker, "asset_type": kind, "average_price": round(new_avg, 6),
                                "quantity": round(new_qty, 6), "status": "holding", "enabled": True, "updated_at": now}); rows[idx] = old
                _save_holdings(rows, sha, f"Update {kind} holding {ticker}")
                st.success(f"{ticker} 저장 완료 · 평균매수가 ${new_avg:,.4f} · 총수량 {new_qty:g}"); st.rerun()
            except Exception as e: st.error(str(e))

    st.markdown("#### 전량 매도 처리")
    tickers = [str(x.get("ticker", "")).strip().upper() for x in active if x.get("ticker")]
    if tickers:
        ...
        if st.button("✅ 전량 매도 → 감시 종료", use_container_width=True, key="hy_hold_close_btn"):
            try:
                ...
                if old is not None:
                    old.update({"status": "closed", "enabled": False, "closed_at": datetime.now(timezone.utc).isoformat()}); rows[idx] = old
                    _save_holdings(rows, sha, f"Close holding {sell}"); st.success(f"{sell} 전량 매도 처리 완료"); st.rerun()
            except Exception as e: st.error(str(e))
